...
/* 
 * [java 접근 제한자]
 * 1) public: 누구라도 접근 가능 (사용가능)
 * 2) protected: 같은 패키지 클래스 접근 가능 + 패키지가 달라도 자식 클래스는 접근 가능
 * 3) default: 접근 제한자를 쓰지 않았을 때 자동적으로 default가 되버림, 같은 패키지에 있는 클래스만 접근 가능
 * 4) private: 클래스 내부에서만 나만 사용 가능, 자식도 접근 불가능
 */
...
//		System.out.println("돈 액수: " + child.money);
