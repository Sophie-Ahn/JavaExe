...
import ch09.resolve14.resolve1.InkjetPrint;
...
public class Answer1 implements IQuestionAnswer {
	...
	public void answer(Scanner sc) {
		sc.nextInt(); // enter치기 전까지 멈춰 있음
		
		InkJetPrinter ijp = new InkJetPrinter("HP-264", "HP", P_INTERFACE.USB, 20, 100, 100);
		ijp.print();
		ijp.print();
		ijp.print();
		System.out.println(ijp);
		
		LaserPrinter lp = new LaserPrinter("Samsung-111", "Samsung", P_INTERFACE.PARALLEL_PORT, 10, 40, 70);
		lp.print();
		lp.print();
		lp.print();
		System.out.println(lp);
		
		sc.close(); // enter치기 전까지 멈춰 있음
	}
