 ...
 * 자연스러운 종속가계이면 상속을 고려할 수 있따.
 * 그렇지 않다면 사용하지 않은 것이 좋다.
 * ex) Gun - Police : 자연스러운 종속관계가 아니라 Plice가 Gund의 기능이 필요한 관계
 * 
 * 2) 만약 이 예제처럼 상속관계를 정한다면
 * 2-1) 불필요한 기능을 제공함으로써 잘못된 서비스/오류를 야기할 가능성이 매우 높아진다.
 * 2-2) 상속간계를 부모의 모든 객체에 포함되므로 메모리의 낭비를 가져 올 수 있다.
 * 
