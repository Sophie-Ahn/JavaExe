 ...
 * - 병렬처리를 위해 Client 1개당 Thread 1개를 배정
 * 3) JsonChatServer
 * - 다양한 업무 처리를 위해서 Json 포맷 사용(패킷)
 * - 채팅은 1사람한테 수신된 데이터를 다른 사람한테 전달해야 한다.
 *   그러므로 사용자 등록/관리가 필요 (HashTable=HashMap+동기화)
 */

public class JsonChatServer {
	public static void main(String[] args) {
		final int PORT = 9000;
		Hashtable clientHt = new Hashtable(); // 접속자를 관리하는 테이
		...
		try {
			...
			while (true) {
				System.out.printf("[서버-%s] client접속 기다리는중><", mainThreadName);
...
class WorkerThread extends Thread {
	...
	public void run() {
		try {
			...
			System.out.printf("<서버-%s> %s로부터 접속함.\n", getName(), inetAddr.getHostAddress());
	...
	private void processPacket(JSONObject packetObj) throws IOException {
		...
		// 사칙연산 업무 결과 요청
			...
			double v2 = Double.parseDouble(val1);
		...
		// 특정 id 대상한테 1:1 채팅
			...
			uniObj.put("id", id);
			uniObj.put("msg", msg);
			String strBroad = uniObj.toString();
			// youid 클라이언트한테 전송
			unicast(strBroad, yourid);
		}
